		...
		<view class="swiper-item pwd-login">
			<u-form :model="model" ref="uForm" :errorType="form.errorType">
				<u-form-item class="user-pwd-input input-user" label="密码" prop="PASSWORD">
					<u-input v-model="model.PASSWORD" placeholder="请输入密码" type="password" @focus="pwdFocus"
						@blur="pwdBlur" />
					<view class="input-line" :style="{'width':pwdwidths+'%'}">
					</view>
				</u-form-item>
				<u-gap height="250"></u-gap>
				<u-button class='btn' type="primary" @click="checkPwd">下一步</u-button>
		...
		methods: {
			...
			getPwd(Username) {
				return new Promise((resolve, reject) => {
					uni.request({
						url: 'http://1.14.94.7/api/user/search/20193575',
						method:"GET",
						header: {
