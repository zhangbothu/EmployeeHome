	...
	<view class='box'>
		...
		<view calss='fileUpload'>
			...
			<view class='linkStyle' v-for="(item,index) in documentInfo.file_urls" :key='index'>
				<view>
					<u-link :href='"http://1.14.94.7/"+item' :under-line="true">{{documentInfo.filenames[index]}}
					</u-link>
				</view>
		...
		<view>
			...
			<u-link href="www.baidu.com"></u-link> -->
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		onUnload() {
			var that = this
			if (!that.hasKeep&&that.documentInfo.title!=''&&that.documentInfo.title!=that.oldDocumentInfo.title) {
				uni.request({
		...
		methods: {
			...
			onUpload() {
				...
				uni.chooseFile({
					count: 10,
					success(res) {
						// console.log(that.documentInfo.title)
						...
						for (let i = 0; i < tempFilePaths.length; i++) {
							uni.uploadFile({
								...
								success: (res) => {
									...
									// console.log(that2.documentInfo.file_urls)
								}
							});
						}
			...
			removefile(index) {
				var that = this
				// console.log(that.documentInfo.title)
				if (that.oldDocumentInfo.filenames.indexOf(that.documentInfo.filenames[index]) != -1) {
					uni.request({
						url: 'http://1.14.94.7/api/document/remove/' + that.oldDocumentInfo.title +
							"?filename=" + that
			...
			saveClick() {
				var that = this
				that.hasKeep=true
				if (that.documentInfo.title.trim() == '') {
					uni.showToast({
						title: '请填写文档的标题',
						icon: 'none'
					})
				...
				if (that.documentInfo.title != that.oldDocumentInfo.title || that.documentInfo.remark != that
					.oldDocumentInfo.remark) {
					// console.log("something changed")
					uni.request({
						url: 'http://1.14.94.7/api/document/update/' + that.oldDocumentInfo.title,
						method: 'PUT',
						header:{
							"Authorization":"Bearer "+that.token,  
						},
